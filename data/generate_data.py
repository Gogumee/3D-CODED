...
def generate_potential_templates(pose, beta, outmesh_path):
    ## Assign random surreal pose parameters
...
def generate_database_surreal(male):
    ...
    #VAL DATA
    nb_generated_humans = 100
    for i in range(nb_generated_humans):
        pose, beta = get_random(poses, betas)
        generate_surreal(pose, beta, 'dataset-surreal-val/' + str(offset + i) + '.ply')
...
def generate_database_benthumans(male):
    ...
    for i in range(nb_generated_humans):
        pose, beta = get_random(poses, betas)
        generate_benthuman(pose, beta, 'dataset-bent-val/' + str(offset + i) + '.ply')
